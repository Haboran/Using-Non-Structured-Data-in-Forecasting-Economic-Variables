...
def forecast_with_sentiment_models_md(series, sentiment_df_monthly, sentiment_df_daily,
                                    country_code, sentiment_vars, sentiment_cols, order,
                                    window_split=(0.4, 0.4, 0.2), forecast_horizon=1, lags=30, plot=False):
    ...
    # mark the index as monthly periods
    series.index = series.index.to_period('M')
    gdp_temp = series.copy()
    gdp_temp = pd.to_numeric(gdp_temp, errors='coerce').dropna().astype(float)
    # convert back to timestamps at month-end
    gdp_temp.index = gdp_temp.index.to_timestamp() + MonthEnd(0)
    ...
    # Coerce y to numeric floats, dropping any non-convertible values 
    y = pd.to_numeric(y, errors='coerce')  
    if y.isna().any():
        print(f"Dropping {y.isna().sum()} NaNs after coercion to float")
        y = y.dropna()   
    ...
    exog_q_df = sentiment_df_monthly.set_index('date').reindex(y.index)
...
#     best_topic = sentiment_vars[0]
#     # build a lagged matrix [lag0 … lag_lags] of that one series
#     daily_df = sentiment_df_daily.set_index("date")
#     midas_col = f"{country_code}_{best_topic}"
#     s = daily_df[midas_col].dropna()

#     # aggregate to month-end to match y.index (or quarter-end if you're in Q)
#     # 1) get last‐of‐month values
#     s_monthly = s.resample('M').last()

# #    2) shift the index to true month‐ends
#     new_idx = s_monthly.index.to_period('M').to_timestamp() + MonthEnd(0)
#     s_monthly.index = new_idx

#     # 3) align to your forecast index
#     s_monthly = s_monthly.reindex(gdp_temp.index)

#     # align to gdp_temp.index (already month-ends)
#     s_monthly = s_monthly.reindex(gdp_temp.index)

#     # now build lags
#     X_lags = pd.concat(
#         [s_monthly.shift(i).rename(f"{best_topic}_lag{i}") for i in range(lags+1)],
#         axis=1
#     )
#     # drop any rows with missing lags
#     X_midas_df = X_lags.dropna()
#     dates_midas = X_midas_df.index
    ...
    # Now guard against empty data
    if X_lasso_full.shape[0] == 0:
        print(">> WARNING: X_lasso_full is empty—skipping scaling/LASSO")
        X_lasso_scaled = pd.DataFrame([], index=X_lasso_full.index, columns=X_lasso_full.columns)
    else:
        X_lasso_scaled = pd.DataFrame(
            StandardScaler().fit_transform(X_lasso_full.values),
            index=X_lasso_full.index,
            columns=X_lasso_full.columns
        )
