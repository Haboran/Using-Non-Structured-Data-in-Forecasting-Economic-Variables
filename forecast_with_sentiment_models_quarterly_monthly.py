# -*- coding: utf-8 -*-
"""
Created on Thu Jun 26 14:31:57 2025
...
def forecast_with_sentiment_models_qm(series, sentiment_df_quarterly, sentiment_df_monthly,
                                    country_code, sentiment_vars, sentiment_cols, order,
                                    window_split=(0.4, 0.4, 0.2), forecast_horizon=1, lags=6, plot=False):
    ...
    # === U-MIDAS with AR lags (direct H-step) using monthly data ===
    midas_col = f"{country_code}_{sentiment_vars[0]}"
    # 1) grab your single monthly sentiment series
    monthly_series = (
        sentiment_df_monthly
        .set_index("date")[midas_col]
        .dropna()
    )
    monthly_series.index = monthly_series.index + MonthEnd(0)
    # 2) build lagged monthly features manually
    #    e.g. if lags=6, you’ll get m_lag_1 … m_lag_6 (i.e. 1–6 months back)
    monthly_lags = {
        f"m_lag_{lag}": monthly_series.shift(lag)
        for lag in range(1, lags + 1)
    }
    monthly_lags_df = pd.DataFrame(monthly_lags)

    # 3) align to your GDP (quarterly) dates
    X_midas_df = monthly_lags_df.reindex(gdp_temp.index).dropna()
    dates_midas = X_midas_df.index

    # 4) build your AR(p) oen-step regressors
    y_series = gdp_temp
    ...
    # 2) build lagged monthly features (m_lag_1 … m_lag_lags)
    monthly_lags = {
        f"m_lag_{lag}": monthly_series.shift(lag)
        for lag in range(1, lags + 1)
    }
    ...
    # 3) align to your GDP (quarterly) dates
    X_lasso_df = monthly_lags_df.reindex(gdp_temp.index).dropna()
    dates_lasso = X_lasso_df.index
    
    # 4) build your AR(p) regressors and align
    y_series = gdp_temp
    ...
    # 6) rolling direct-H LASSO forecasts
    ...
    # 1) build the lag-matrix from monthly_series
    monthly_series = sentiment_df_monthly.set_index("date")[midas_col].dropna()
    monthly_series.index += MonthEnd(0)
    
    X_r_list, dates_r = [], []
    for d in gdp_temp.index:
        # grab the last `lags` months up to quarter-end d
        window = monthly_series.loc[d - pd.DateOffset(months=lags-1) : d]
        if len(window) >= lags:
            # tail(lags) ensures exactly `lags` months; reversed so index 0 = most recent
    ...
    # 2) build AR(p) lags and align to those dates
    y_lags_r = pd.DataFrame(
        {f"y_lag_{lag}": y_series.shift(lag) for lag in range(1, p+1)},
        index=y_series.index
    ).reindex(valid_r)
    
    # 3) estimate Almon weights on the training subsample
    T_r = len(valid_r)
    ...
    # 4) form the projected MIDAS regressor + AR lags
    ...
    # 5) rolling direct-H forecasting
